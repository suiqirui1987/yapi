...
module.exports = function batchToHtml(reports, data) {
  let tp = createHtml(reports, data);
  return tp;
};

function createHtml(reports, data) {
  let mdTemplate = ``;
  let left = ``;
  let left_left = ``;
  ...
  reports.map((item,index) => {
    left_left += `
            <div class="list-content">
              <a class="list" href="javascript:batchexec(${index});">${item.name}</a>
              <div id="status_${item._id}"></div>
            </div>
    `;
  });
  ...
  return createHtml5(reports, data, left_left);
}



function createHtml5(reports, data, left_left) {
  let message = ``;
 
  reports_str = JSON.stringify(reports);
 ...
 window.reportarr = ${reports_str};
 window.globaldata = ${data_str};
  ...
  <body class="yapi-run-auto-test">
    <div class="m-header">
      ...
      <div class="nav">
          <input type="button" id="batchexecseverbtn" onclick="batchexecsever()" value="自动化测试" />
      </div>
    </div>
    <div class="g-doc">
      ...
      <h1>YApi 测试报告</h1>
      <div class="summary">  <div id="message"></div></div>
       <div id="tp"></div>
